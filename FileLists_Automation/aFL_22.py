...
for doc in Docs:
	#The next two conditionals have filenames where Control File and its Doc do not simply differ by capitalization.
	if (doc in 'taxrfnds'):
...
for file in Files_noInts:
	...
	if (file == 'taxholdy'):
		...
		trouble_names = set(['taxholdy','taxholdy2','taxholpd','taxholpd2','TAXHOLDY','TAXHOLPD'])
		FD_noInts = set(Files_noInts+Docs_noInts)
...
#Add changing Docs. Then, check which Docs and Files were missed, based on existing files.
...
#trouble_names = set(['proddesc','proddescref.txt','proddesc.txt'])
#Docs_noInts = set(['proddesc']+C_incrementalDocuments+Cobol_incrementalDocuments)
#intersection_proddesc = Docs_noInts.intersection(trouble_names)
#if (intersection_proddesc != trouble_names):
#	print("Intersection Proddesc is missing related file(s)!")
#del Docs_noInts[-1]

print('taxupdt_'+taxupdate_no, 'verataxPLUS.'+currentNoMonth+currentYear[2:]+'.txt')
trouble_names = set(['taxupdt_'+taxupdate_no,'VERATAX.TXT','verataxPLUS.'+currentNoMonth+currentYear[2:]+'.txt','txupdtno','TMDFL','TxManagerInstaller','TAXMGR.txt'])
intersection_taxupdt = Files_noInts.intersection(trouble_names)
#if (intersection_taxupdt != trouble_names):
#	print("Intersection Taxupdt is missing related file(s)!")
...
tree_indented.write("C:/Users/jeffrey.thomas/Desktop/New_FileLists_indented_1.xml")
